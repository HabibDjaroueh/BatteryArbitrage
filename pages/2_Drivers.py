...
def main():
    ...
    with left_col:
        available_spreads = [c for c in REGION_SPREADS.get(region, []) if c in df.columns]
        ...
        target_col = st.selectbox(
            ...
            format_func=lambda x: (
                "Spread: " + x.replace("spread_", "").replace("_", " → ").upper()
                if "spread" in x
    ...
    if (results["target_col"] != target_col
        ...
        st.warning(
            "⚠️ Model configuration has changed since last training. "
    ...
    m3.metric("Direction Acc",  f"{results['direction_acc']:.1f}%"
                                if results["direction_acc"] is not None else "N/A")
    ...
    st.subheader(f"🏆 Top {top_n_features} Feature Importances")
